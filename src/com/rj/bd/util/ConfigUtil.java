    ...
    public static String getConfig(String key, boolean cacheFlag) {
    	...
    	try {
    		...
    		if(cacheFlag && gConfigUtilConfigCachePool.contains(savekey)) {
                ...
                String sql = "SELECT SysValue FROM sys_config WHERE `ConKey`='"+key+"' AND (ServerName='' OR ServerName is null OR ServerName='"+serverName+"')  AND Status=1 ORDER BY ServerName DESC";
