...
public class DataTable {
	 ...
	 * @throws Exception
	 */
	public com.alibaba.fastjson.JSONArray getAliJSONArray() throws Exception{
		...
		//转换DataTable为JSONArray
		com.alibaba.fastjson.JSONArray jarr = new com.alibaba.fastjson.JSONArray();
		for(int i=0; i<rows.size(); i++){
			com.alibaba.fastjson.JSONObject curObj = new com.alibaba.fastjson.JSONObject();
			for(int c=0; c<columns.size(); c++){
				String colName = columns.get(c);
				Object colValue = getCell(i, c);
/*				if (("AddTime").equals(colName)) {
					Field[] fields = colValue.getClass().getDeclaredFields();
					for (int j = 0; j < fields.length; j++) {
						LogUtil.log("xxx" + fields[j].getType());
					}
				}*/
				curObj.put(colName, colValue);
			}
